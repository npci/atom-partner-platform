...
cp backend/.env.example backend/.env    # set SESSION_JWT_SECRET and ADMIN_PASSWORD; LLM key optional
docker compose up -d
```

Open <https://localhost:8443/a2a-partner/>. Log in as `admin` with the
`ADMIN_PASSWORD` you set — there is no default; the service refuses to seed an
admin user without one.
...
installs from the lock; see [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md)
...
- **Never interpolate a settings key into a log line — wrap it in
  ...
  (`authority_hmac_secret`, `partner_api_key`, …) must be passed through
...
The seven shipped agents are reference implementations. In your own fork you will
