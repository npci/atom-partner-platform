...
Concretely, the middleware here differs from the Authority's counterpart in
...
- **A database-backed nonce store for replay.** Most partner deployments do
  not run Redis, so the nonce store falls back to Postgres
  (`a2a_common/nonce_store_db.py`), with Redis as an optional backing where
  deployed. Either way it fails closed.

The details of that last one are covered in
