...
`partner_settings.authority_jwt_secret` is absent, the middleware answers `503`
to inbound A2A calls. The same holds for `authority_hmac_secret`.
...
| **Database-backed nonce store** (not Redis-only) | Most partner deployments do not run Redis, so the store falls back to Postgres | A DB round-trip per verified request; Redis remains the faster backing where deployed |

The third is no longer a reduction: a captured, correctly-signed message is
rejected on replay, and the store fails closed. The wiring lives in
`nonce_store_db.build_nonce_store()`, which picks Redis when configured and
Postgres otherwise.
...
| `503` from the A2A mount | JWT | No `authority_jwt_secret` configured — onboarding is incomplete |
...
The architecture decision records these behaviours were originally specified in
did not come across in the repository split. Rather than leave every module
pointing at a file nobody can open, the reasoning was folded into the module
docstrings themselves, and the dangling citations were removed (2026-09-07).

So the docstrings ARE the authority — not a stand-in for one. If you change
fail-closed ingress or the hostility tiers, update the explanation where it now
