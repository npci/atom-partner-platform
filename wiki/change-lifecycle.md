...
| **Counter-proposal** | `authority_counter_history`, `counter_decisions` | Round-based, append-only |
| **Progress** | `progress_reports` | Coarse milestones |

**The negotiation history is append-only on purpose.** `authority_counter` holds
the *active* counter and is cleared to `NULL` when answered;
`authority_counter_history`
