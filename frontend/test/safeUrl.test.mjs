...
//
// Guards the remediation for the static-analysis findings "DOM XSS",
// "reflected XSS", and "DOM open redirect" — all three of which fired on
...
// The previous sanitiser matched the host with /^https?:\/\/([^\/?#:]+)/,
...
// is why a taint engine still saw an unbroken reflected-XSS
...
// The two findings are XSS reports, so assert the XSS vectors
...
// reported line-352 sink) has no error boundary, making that a white-screen DoS
...
// reaches the href. That is why static analysis re-reported DOM XSS at
