...
// ── WHY THIS EXISTS ─────────────────────────────────────────────────────────
//
// A static-analysis "unchecked input for loop condition" finding reported:
//
...
// value. `Math.min` is not a sanitiser in the analyser's model; it is ordinary
...
// gets a failing build instead of a scanner ticket months later.
...
for (const file of files) {
  ...
  for (const { header, line } of forHeaders(code)) {
    ...
    failures.push(`${rel}:${line} loops until "${bound}", which is not a `
      + `build-time constant.\n`
      + `      why banned: the unchecked-input-for-loop-condition check reads the `
...
const REQUIRED_CAPS = [
  {
    ...
    why: 'Bounds the DETAILS-block line scan in _detailField — the exact sink '
       + 'named in the reported finding (ChangeDetail.jsx:134, source '
...
//
// This is the specific pattern that was already in the code when the scan
...
if (failures.length === 0) {
  console.log(
    'All loops are bounded by build-time constants. The '
    + 'unchecked-input-for-loop-condition finding (ChangeDetail.jsx _detailField) stays closed.'
  );
}
