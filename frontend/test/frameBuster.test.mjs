...
// ── WHY THIS EXISTS ─────────────────────────────────────────────────────────
//
// A "clickjacking on legacy browsers" finding reports against
// frontend/index.html line 1. It is a STATIC check
...
// instead of silently reopening the finding on the next scan.
//
...
// also be invisible to the static check, so the finding would recur.
...
check(
  ...
  + 'framed page from painting, and is not visible to the static '
...
// Best-effort only — a cross-origin parent blocks it — but standard guidance
