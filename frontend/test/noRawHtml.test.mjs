...
// WHY THIS EXISTS
//
// Static analysis reported reflected XSS (CWE-79) against two `href` sinks:
...
// the static-analysis scan itself are the controls there. Do not read a green run as
  ...
  for (const m of code.matchAll(URL_ATTRS)) {
    ...
    if (isStaticLiteral || APPROVED_URL_WRAPPERS.test(expr)
      ...
      failures.push(
        ...
        + `This is the exact sink pattern static analysis flagged. `
