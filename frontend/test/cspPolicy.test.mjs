...
// ── WHY THIS EXISTS ─────────────────────────────────────────────────────────
//
// A "permissive Content Security Policy" static-analysis finding reported
...
// ways nobody tested.
{
  ...
  if (metaCsp.test(withoutComments)) {
    fail(
      ...
      + '      This is the exact line flagged as a permissive Content\n'
      + '      Security Policy. frame-ancestors is IGNORED in a meta\n'
  ...
  if (!/id="antiClickjack"/.test(withoutComments)) {
    fail('index.html no longer contains the antiClickjack frame buster — the '
      + 'clickjacking query (a static check on this file) would '
...
// static-analysis findings behind each choice. Useful to a maintainer, but handing
// the public a map of the app's security controls is itself
  ...
  if (!existsSync(DIST_INDEX)) {
    ...
    if (/<!--/.test(dist)) {
      fail('dist/index.html still contains HTML comments. index.html documents '
        + 'the security controls and the static-analysis findings; shipping that '
