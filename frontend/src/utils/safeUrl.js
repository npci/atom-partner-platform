...
// Any taint-tracking static analyser therefore still sees an unbroken
// source→sink path and reports XSS / open
// redirect, because "this string was compared against some constants" is not a
// sanitisation step a taint engine can recognise. Three separate analyser
// queries fired on exactly that shape: DOM XSS, reflected XSS, and
// DOM open redirect.
//
...
// remediation the static-analysis report recommends, rather than a suppression.
//
...
//      source→sink path alive even though this filter blocked the
 ...
 * — it kept the source→sink edge alive and is why static analysis re-reported
 * these `href` sinks as DOM XSS even after the sanitiser was hardened.
 */
...
function rebuildTail(tail) {
  ...
  // static analysis flags as an unchecked input controlling iteration.
 ...
 * why static analysis continued to report these `href` sinks as reflected XSS even
