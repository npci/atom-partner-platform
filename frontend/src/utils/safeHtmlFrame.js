...
/**
 * First match of `re` that does not fall inside an HTML comment.
 *
 * The insertion point used to be located with a plain `.match(/<head[^>]*>/i)`,
 * which happily matched text inside `<!-- ... -->`. Content carrying
 * `<!-- a <head> in prose -->` before its real `<head>` therefore had the
 * policy spliced INTO the comment, where it is inert — and because the branch
 * returned there, the document ended up with no CSP at all. That is precisely
 * the "content decides its own hardening" failure the rest of this module was
 * written to avoid, reintroduced through the search rather than the check.
 *
 * A comment cannot nest and is closed by the first `-->`, so a single forward
 * scan is enough; this deliberately does not attempt to be a full parser.
 *
 * @param {string} s
 * @param {RegExp} re global regex to search with
 * @returns {RegExpExecArray | null}
 */
function matchOutsideComments(s, re) {
  re.lastIndex = 0;
  let m;
  while ((m = re.exec(s)) !== null) {
    const before = s.lastIndexOf('<!--', m.index);
    if (before === -1) return m;                 // no comment opens before it
    const closed = s.indexOf('-->', before);
    if (closed !== -1 && closed < m.index) return m;  // that comment already closed
    // Inside an open comment — keep looking after it ends.
    re.lastIndex = closed === -1 ? s.length : closed + 3;
  }
  return null;
}
...
export function hardenFrameHtml(html) {
  ...
  const headOpen = matchOutsideComments(stripped, /<head[^>]*>/gi);
  if (headOpen) {
  ...
  const htmlOpen = matchOutsideComments(stripped, /<html[^>]*>/gi);
  if (htmlOpen) {
