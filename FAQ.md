...
The Authority's JWT and HMAC secrets, the partner API key, the LLM key and the
GitLab token are encrypted at rest in `partner_settings` under a key derived from
...
auth message would pin the resolved authority host, port and token prefix into a
