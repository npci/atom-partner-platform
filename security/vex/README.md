...
- `sqlalchemy 2.0.36 → 2.0.52` was tested and the backend suite passes against
  ...
  version-independent, and at the time it would have deepened the lockfile
  drift described below.
  The reasoning is recorded inline at the pin in `backend/requirements.txt`.

## Historical note: lockfile drift, found during this work and since resolved

During this work `requirements.lock` was found to have drifted from
`requirements.txt` (stale `fastapi`/`starlette`, `python-jose` and `ecdsa`
still present, `pyjwt` absent). The lock has since been regenerated and the
two files now agree — `pyjwt 2.13.0`, `fastapi 0.141.1` and `starlette 1.3.1`
are in the lock, and `python-jose`/`ecdsa` are gone. See
`THIRD-PARTY-NOTICES.md` for the reconciliation record;
`backend/tests/test_requirements_lock_sync.py` keeps the two files from
drifting apart again. This section is retained as history, not as a live
defect.
...
These run in CI on GitHub Actions: `.github/workflows/ci.yml` (whose
`backend-tests` job is blocking) and `.github/workflows/secret-scan.yml`.
...
**Rotating `authority_jwt_secret` requires coordination with the Authority.** It is a shared
key, and the A2A ingress is fail-closed by design (see the module docstring in
`backend/app/a2a_common/auth_middleware.py`), so a unilateral
