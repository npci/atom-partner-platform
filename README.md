...
> **Windows + Git Bash:** prefix the `openssl` line with `MSYS_NO_PATHCONV=1`.
> Git Bash rewrites the `/CN=localhost` argument into a Windows path, and
> OpenSSL then rejects it with *"subject name is expected to be in the format
> /type0=value0/… This name is not in that format:
> `C:/Program Files/Git/CN=localhost`"* — a message that names the mangled
> value without ever saying the shell did it.
>
> ```bash
...
> ```
>
> The prefix is inert on Linux and macOS, so that one line is safe to use
> everywhere. PowerShell does no such rewriting — the original command works
> there as written.
...
- **One of the seven agents (`negotiation`) is a stub.** That is by design — the
  agents are the parts you replace — and without an LLM key even the real ones
  return mock output, so a fresh clone demonstrates the flow rather than
