...
def change(db_session):
    row = IncomingChange(id=LOCAL_ID, authority_change_id=NPCI_ID, title="A change")
    db_session.add(row)
    db_session.commit()
    return row
...
def test_the_rig_is_still_told_the_authoritys_change_id(db_session, change, monkeypatch):
    """`cert_context.authority_change_id` travels back OUT on the wire, where only
    ...
    assert fired[0]["cert_context"]["authority_change_id"] == NPCI_ID
...
def test_an_unknown_change_falls_back_to_the_envelope_id(db_session, caplog):
    ...
    assert reply["case_data"]["TC1"]["account"] == "42"
    assert "no IncomingChange for authority_change_id" in caplog.text
