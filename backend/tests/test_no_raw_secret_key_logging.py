...
--------------------
This is the enforcement half of the "Filtering Sensitive Logs"
static-analysis remediation. That query has now been reported twice against
this repository:

  - five paths, on log calls of the form
        logger.critical("failed to decrypt %s", key)
  - six paths, after the first fix routed `key` through a constant
    lookup table that the scanner treats as a taint PROPAGATOR rather
    than a barrier.

Both rounds were false positives — the object logged was always the setting's
NAME ("authority_hmac_secret"), never the secret VALUE. But a finding that is
...
literally or held in a variable:
    ...
    logger.warning("rotating %s", "authority_hmac_secret")   # literal
    k = "authority_hmac_secret"; logger.warning("...", k)    # via a local
...
and the secret VALUE itself, read out of a settings row or request body:
    ...
    logger.info("saved %s", body.authority_jwt_secret)
...
def test_no_raw_secret_key_logging(path: Path):
    ...
    if violations:
        ...
        pytest.fail(
            ...
            "the pattern behind the 'Filtering Sensitive Logs' finding, which "
            "has already been reported against this repo twice.\n"
            f"{detail}\n\n"
...
def f():
    logger.warning("rotating %s", "authority_hmac_secret")
''',
    "key via local": '''
import logging
logger = logging.getLogger(__name__)
def f():
    k = "authority_jwt_secret"
...
logger = logging.getLogger(__name__)
def f():
    logger.info(f"touching {"authority_hmac_secret"}")
''',
    "dict lookup keyed by a secret": '''
import logging
logger = logging.getLogger(__name__)
def f(table):
    logger.info("label %s", table["authority_hmac_secret"])
''',
}
...
logger = logging.getLogger(__name__)
def f(body):
    logger.info("saved (len=%d)", len(body.authority_jwt_secret))
''',
...
def test_guard_reports_line_and_function():
    ...
    assert line == 4
    assert func == "handler"
    assert "authority_hmac_secret" in why
