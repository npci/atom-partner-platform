...
"""Tests for the outbound A2A retry sweep (DLQ and replay process)."""
...
class TestRunSweepDelivery:
    def test_delivers_due_row_and_marks_delivered(self, db_session, monkeypatch):
        ...
        monkeypatch.setattr("app.authority_client._dispatch_wire", fake_dispatch)
    ...
    def test_ignores_rows_not_yet_due(self, db_session, monkeypatch):
        ...
        monkeypatch.setattr("app.authority_client._dispatch_wire", lambda *a, **kw: None)
        counts = outbound_retry.run_sweep(db_session, max_attempts=6)
...
class TestRunSweepRequeue:
    def test_failed_attempt_requeues_with_backoff(self, db_session, monkeypatch):
        ...
        monkeypatch.setattr("app.authority_client._dispatch_wire", _fail)
        counts = outbound_retry.run_sweep(db_session, max_attempts=6)
        ...
        assert row.status == "pending"
        assert row.attempts == 1
        # `last_error` is rendered in the retry-queue UI, so authority_client records
