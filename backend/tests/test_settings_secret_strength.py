...
Why the boundary matters more than the unit here: `authority_jwt_secret` is the HS256
key that `PartnerAuthMiddleware` verifies every inbound A2A call against, and
`authority_hmac_secret` drives request signing. Neither can be checked at startup
...
@pytest.mark.parametrize("field", ["authority_jwt_secret", "authority_hmac_secret"])
...
def test_weak_secret_is_rejected_with_400(client, field, weak):
    ...
    for label_word in ("authority JWT secret", "authority HMAC secret", "secret"):
...
def test_empty_value_still_means_leave_unchanged(client):
    ...
    assert resp.status_code == 200, resp.text
    assert "authority_jwt_secret" not in resp.json()["persisted"]
...
def test_rejection_does_not_partially_persist_other_fields(client):
    ...
    resp = client.put(
        "/api/settings",
        json={"partner_name": "ShouldNotStick", "authority_jwt_secret": "weak"},
    )
    assert resp.status_code == 400, resp.text
