...
2. FALSE POSITIVES — a legitimately generated key is rejected. On this codebase
   that is arguably worse: `authority_jwt_secret` is issued by NPCI and validated on
...
def test_error_message_never_echoes_the_secret():
    ...
    These strings land in logs and (for `authority_jwt_secret`) in an HTTP 400 body.
    ...
    """
    # Built rather than written out: a literal here is the shape static
    # analysis reports as a hardcoded password (see
