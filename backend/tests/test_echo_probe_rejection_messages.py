...
from app.authority_client import _send_echo_probe
...
def probe(monkeypatch):
    ...
    def _run(exc):
        ...
        monkeypatch.setattr("app.authority_client._run_portably", boom)
...
def test_a_secret_missing_on_the_authority_side_says_so(probe):
    """Signed here, but the authority holds no secret for this partner — the
    remedy is on the authority, not in this platform's Settings."""
    ok, detail = probe(_sdk_error(401, {"error": "hmac_secret_not_configured"}))
    assert ok is False
    assert "authority" in detail and "NO signing secret" in detail
...
def test_the_installed_sdk_really_does_preserve_the_response_body():
    ...
    from app.authority_client import _CLOCK_SKEW_MSG, _describe_rejection
