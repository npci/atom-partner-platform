...
def stub_wire(monkeypatch):
    ...
    monkeypatch.setattr(authority_client, "_dispatch_wire", fake_dispatch)
    return calls
...
def test_the_reply_body_comes_home(stub_wire, db_session):
    assert authority_client.send_task(db_session, "echo", None, {}) == RECEIPT
...
def test_success_without_an_artifact_keeps_the_delivery_marker(monkeypatch, db_session):
    ...
    monkeypatch.setattr(authority_client, "_dispatch_wire", fake)
    assert authority_client.send_task(db_session, "echo", None, {}) == {"status": "delivered"}
...
def test_transport_failure_returns_none_and_queues_the_retry(monkeypatch, db_session):
    ...
    monkeypatch.setattr(authority_client, "_dispatch_wire", fake)
    assert authority_client.send_task(db_session, "echo", "chg-1", {"x": 1}) is None
...
def test_async_caller_awaits_the_sender_under_a_running_loop(stub_wire, db_session):
    ...
    async def scenario():
        return await authority_client.send_task_async(db_session, "echo", None, {})
...
def test_sync_facade_refuses_the_event_loop(stub_wire, db_session):
    ...
    async def scenario():
        authority_client.send_task(db_session, "echo", None, {})
...
def test_anyio_worker_sends_ride_the_application_loop(stub_wire, db_session):
    ...
    async def scenario():
        ...
        return await to_thread.run_sync(
            lambda: authority_client.send_task(db_session, "echo", None, {}))
...
def test_plain_thread_sends_use_isolated_loops_no_reuse(stub_wire, db_session):
    ...
    def worker(i):
        results[i] = authority_client.send_task(db_session, "echo", None, {})
