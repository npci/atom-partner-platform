...
--------------------
A static-analysis "Use Of Hardcoded Password" (Low) rule has been reported
against this repository in three consecutive scans:

  1. paths 1-2, on `_SETTING_NAME = "authority_jwt_secret"` in
     a2a_common/auth_middleware.py and the matching line in
     hmac_middleware.py.
  2. the same two paths, plus five on test fixtures in
     tests/test_secret_box.py and tests/test_a2a_fail_closed.py.
     Triaged as false positives and closed.
  3. all seven returned, joined by an eighth on `PASS = "PASS"` in
     core/test_status.py -- a line that exists ONLY because it was the
     previous remediation for this very query.

Every path was a genuine false positive. `authority_jwt_secret` is a `partner_settings`
...
def test_no_credential_named_string_literal(path: Path):
    ...
    if offenders:
        ...
        pytest.fail(
            "Credential-shaped string literal. Static analysis reports this "
            "exact shape as 'Use Of Hardcoded Password', and it has recurred in "
...
def _is_settings_row_key(text: str) -> bool:
    ...
    `_get_setting(db, "authority_hmac_secret")` is the legitimate way to read a row
...
def test_no_secret_looking_value_literal(path: Path):
    ...
    if offenders:
        ...
        pytest.fail(
            ...
            "round-trip can no longer pass because of one fixed string."
        )
...
def test_enum_members_use_auto_not_literals(module: str, class_name: str):
    ...
    `authority_jwt_secret = auto()` as `authority_jwt_secret = "authority_jwt_secret"` is
...
def test_setting_key_values_are_unchanged():
    ...
    `SettingKey.authority_jwt_secret == "authority_jwt_secret"` one line at a time. Both
    ...
    assert set(SettingKey) == {
        ...
        "authority_platform_url", "authority_a2a_url", "partner_name",
        "cert_trigger_url",
    }
    ...
    assert SettingKey.authority_jwt_secret in set(SECRET_SETTING_KEYS)
    assert {SettingKey.authority_jwt_secret: 1}["authority_jwt_secret"] == 1
    assert f"{SettingKey.authority_hmac_secret}" in set(SECRET_SETTING_KEYS)
...
def test_middleware_setting_name_comes_from_the_registry(module: str):
    ...
    for value in bindings:
        assert isinstance(value, ast.Attribute) and isinstance(value.value, ast.Name), (
            ...
            "(e.g. SettingKey.authority_jwt_secret), not a string literal."
        )
