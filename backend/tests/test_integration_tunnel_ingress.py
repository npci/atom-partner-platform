...
def _stub_send(monkeypatch, reply):
    ...
    monkeypatch.setattr(authority_client, "send_task_async", fake)
    return calls


def _forward(**kw):
    defaults = dict(db=None, alias="authority_simulator", method="POST",
...
def test_the_exchange_payload_and_budget_ride_the_wire(monkeypatch):
    ...
    assert payload["target"]["alias"] == "authority_simulator"
...
def test_callback_scenario_end_to_end_through_the_real_contract(monkeypatch):
    ...
    monkeypatch.setattr(settings, "integration_testing_allowlist",
                        '{"authority_simulator": {"scheme": "http", "host": "sim.internal",'
    ...
    monkeypatch.setattr(authority_client, "send_task_async", wire)
