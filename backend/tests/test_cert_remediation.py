...
def _sent(monkeypatch, reply):
    ...
    monkeypatch.setattr(authority_client, "send_cert_fix_notification", fake)
    return calls
...
def test_failed_send_leaves_the_round_parked_not_approved(db_session, monkeypatch):
    ...
    worse failure — a send that returns None (authority_client's failure shape)
...
def test_approval_endpoint_is_the_only_caller_of_the_notification():
    ...
    site outside authority_client itself — the approval endpoint."""
    import pathlib

    root = pathlib.Path(__file__).resolve().parents[1] / "app"
    callers = []
    for py in root.rglob("*.py"):
        if py.name == "authority_client.py":
            continue
        text = py.read_text()
