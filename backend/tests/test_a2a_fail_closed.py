...
"""Tests for the fail-closed A2A ingress default and the inbound body-size
limit.
...
def _fresh_secret(label: str) -> str:
    ...
    static analysis reported as an embedded credential. No real credential was
    ever involved, but the finding returns on every rescan, so the literal is
    gone.
...
class TestHmacMiddlewareFailClosed:
    ...
    def test_valid_signature_passes(self, db_session):
        ...
        _set_setting(db_session, SettingKey.authority_hmac_secret, secret)
        client = _hmac_app(max_body_bytes=1_000_000)
    ...
    def test_invalid_signature_rejected(self, db_session):
        _set_setting(db_session, SettingKey.authority_hmac_secret, _fresh_secret("hmac"))
        client = _hmac_app(max_body_bytes=1_000_000)
        body = json.dumps({"x": 1}).encode()
...
class TestAuthMiddlewareFailClosed:
    ...
    def test_valid_jwt_passes(self, db_session):
        ...
        _set_setting(db_session, SettingKey.authority_jwt_secret, secret)
    ...
    def test_invalid_jwt_rejected(self, db_session):
        _set_setting(db_session, SettingKey.authority_jwt_secret, _fresh_secret("jwt"))
        client = _auth_app()
