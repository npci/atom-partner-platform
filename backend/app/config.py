...
class Settings(BaseSettings):
    ...
    # Authority platform URL — MUST use https:// in any deployment carrying
    # real credentials. The http:// default is accepted only in development for
    # local docker-compose setups; production and staging raise a hard error
    # (see startup guard below). Set AUTHORITY_PLATFORM_URL.
    #
    # The superseded NPCI_PLATFORM_URL is still accepted, and that alias is not
    # a courtesy. An env var name lives in compose files and operators' own
    # `.env`s, outside this repo — and pydantic-settings IGNORES an unknown env
    # var rather than rejecting it (unlike an unknown dotenv KEY, which it
    # forbids). So renaming the field alone does not fail: the old variable is
    # silently disregarded and the field falls back to `http://localhost`,
    # pointing the platform at itself with nothing logged.
    authority_platform_url: str = Field(
        default="http://localhost",
        validation_alias=AliasChoices("authority_platform_url", "npci_platform_url"),
    )
    partner_api_key: str = ""
    # Main partner DB — moved from SQLite to Postgres. MUST be set via the
    ...
    database_url: str = ""
    port: int = 8001
    # Public URL this platform advertises in its AgentCard's
    # supported_interfaces[0].url. The A2A SDK posts JSON-RPC to this URL
    # verbatim — httpx requires the http:// prefix, so a bare path won't work.
    # MUST use https:// in any deployment carrying real credentials.
    #
    # The default is the HOST-deployment answer. It used to be
    # `http://partner_backend:8001` — a container name, and a PRE-RENAME one at
    # that, so even under compose it stopped resolving once the container was
    # renamed: the authority fetched the card successfully and then failed to
    # connect, which reads as this platform being down rather than as a stale
    # advertisement. Compose sets PARTNER_PUBLIC_URL explicitly, so docker is
    # unaffected; the mode that needs no configuration is now the one that
    # cannot resolve a container name.
    partner_public_url: str = "http://localhost:8001"

    # ── SSRF guard for the authority connectivity probe ──────────────
    # `authority_client._is_private_url` refuses to probe a URL that resolves into
    # private or reserved address space, so an operator cannot point
    # `authority_platform_url` at an internal service and use Settings → Test
    ...
    # perfectly legitimate deployment unreachable: the authority's own UAT platform sits
    ...
    # `ssrf_allowed_internal_hosts` / `ssrf_allow_private_networks` in the
    # authority backend's `core/ssrf_guard.py`.
    #
    ...
    #   AUTHORITY_SSRF_ALLOWED_HOSTS=10.84.12.34,authority-uat.internal
    #
    # Both accept their superseded `NPCI_SSRF_*` spellings, for the same reason
    # as `authority_platform_url` above: these names live in compose files and
    # operators' own `.env`s, and pydantic-settings silently IGNORES an unknown
    # env var. Renaming the field alone would not fail — the allowlist would
    # quietly empty itself and every approved private target start refusing
    # again, which reads as the authority being unreachable.
    authority_ssrf_allowed_hosts: str = Field(
        default="",
        validation_alias=AliasChoices(
            "authority_ssrf_allowed_hosts", "npci_ssrf_allowed_hosts"),
    )
    ...
    # legitimate authority platform, so this flag does NOT re-enable it.
    authority_ssrf_allow_private_networks: bool = Field(
        default=False,
        validation_alias=AliasChoices(
    ...
    # Mirrors the authority backend's `core/llm.py` shape: pick the provider
    # via `LLM_PROVIDER` env, then per-provider key + model + (for AiNxt)
    # base_url. Trimmed to three providers — partner stack doesn't run
    # Ollama. ``claude`` remains accepted as a backward-compatible alias.
    llm_provider: str = "anthropic"  # anthropic | openai | ainxt
    ...
    # AiNxt — internal gateway. Two wire modes (mirrors the authority backend):
    ...
    # ── RAG / embeddings (Code-RAG + Document-RAG) ───────────────────
    ...
    # Both the Document RAG (authority docs + knowledge base) and the Code RAG
    ...
    # ── Code RAG — partner's own GitLab repo ─────────────────────────
    ...
    # Local login-session signing key (distinct from the authority-issued
    # `authority_jwt_secret` used for inbound A2A). MUST be set per deployment;
    ...
    # Dashboard REST throttle — PER CALLER per minute, not global. The A2A
    # limiter above is installed on the /a2a-rpc sub-app, and Starlette
    # middleware on a mount does not reach the parent, so the dashboard and
    # auth routers had no limit at all. Generous on purpose: this is a backstop
    # against a stolen session spraying job creation, not a quota, and a
    # normal page load fires a burst of reads that must never trip it.
    dashboard_rate_limit_per_min: int = 600
    # Aliased like the other renamed settings: an operator may already be
    # tuning these through the `NPCI_*` env names, and pydantic-settings would
    # ignore the old spelling silently, snapping the value back to the default
    # with nothing logged.
    authority_outbound_timeout_s: float = Field(
        default=30.0,
        validation_alias=AliasChoices(
            "authority_outbound_timeout_s", "npci_outbound_timeout_s"),
    )
    authority_cb_failure_threshold: int = Field(
        default=5,
        validation_alias=AliasChoices(
            "authority_cb_failure_threshold", "npci_cb_failure_threshold"),
    )
    authority_cb_cooldown_s: float = Field(
        default=30.0,
        validation_alias=AliasChoices(
            "authority_cb_cooldown_s", "npci_cb_cooldown_s"),
    )
    authority_outbound_max_concurrent: int = Field(
        default=10,
        validation_alias=AliasChoices(
            "authority_outbound_max_concurrent", "npci_outbound_max_concurrent"),
    )
    ...
    # calls if authority_jwt_secret/authority_hmac_secret are not yet configured. Defaults
    # to false (fail-closed). NEVER set this in a production or staging
    # deployment.
    partner_allow_unauthenticated_a2a: bool = False

    # Per-change LLM token budget (cost is a bounded resource).
    # 0 = unlimited (explicit opt-out).
    llm_token_budget_per_change: int = 2_000_000

    # ── Database connection pool (externalize infrastructure values
    ...
    # ── Data retention / purge policy (TTL, validity, archival rules).
    # 0 = disable that sweep entirely (explicit opt-out). See
    ...
    # ── Outbound A2A retry sweep (DLQ for partner->authority sends) ─────────
    ...
    # ── Bounded structures / safe scale-in ───────────────────────────────────
    ...
    # marking stragglers interrupted (safe scale-in with drain/linger
    ...
    # Optional, disabled-by-default test-generation agent
    # (agents/test_files.py). Off by
    ...
    # ── Reverse-proxy depth, for attributing a client IP ─────────────────
    # How many of OUR proxies sit between the internet and this process. 0
    # means the process is reached directly and `request.client.host` is the
    # client. 1 is the shipped compose shape (the `partner_edge` nginx).
    #
    # This exists because `api/auth.login` keys a lockout tier on the client
    # address, and behind the edge that address is the nginx container for
    # EVERY user — one shared bucket, so ten failed logins from any anonymous
    # caller locked out every operator, indefinitely and without credentials.
    # Rather than trust `X-Forwarded-For` unconditionally (forgeable by anyone
    # who can reach the backend directly, which the loopback publish in
    # docker-compose.override.yml permits), the depth is declared and the
    # header is read positionally from the right. When it cannot be read, the
    # IP tier is skipped rather than guessed — see `api.auth._client_ip`.
    partner_trusted_proxy_hops: int = 0
    ...
    # authority_client._get_setting swallows the decrypt error and returns the
    ...
    # budget, mirroring the authority side's 105 → 90 → 60. `ingress_timeout_s` is
    ...
    # the two authority-issued secrets is already on the inbound request path. It
...
# unauthenticated calls when the authority secrets are not configured. It is a
...
if settings.partner_allow_unauthenticated_a2a and _env_is_protected():
    raise RuntimeError(
        ...
        "This accepts unauthenticated A2A calls whenever the authority JWT or "
        "HMAC secret is unset, which removes the authentication boundary "
        "entirely. It is a development-only escape hatch.\n"
        "  * Deploying for real: unset PARTNER_ALLOW_UNAUTHENTICATED_A2A and "
        "configure authority_jwt_secret and authority_hmac_secret in partner_settings.\n"
        "  * Local development: set APP_ENV=development."
    )

# ── Refuse the integration-testing tunnel outside development (ITA-4) ──
# `main.py` has long claimed this guard existed — "hard-disabled unless
# integration_testing_enabled, which production config refuses" — but nothing
# tied the flag to APP_ENV. Only per-request checks existed, so
# INTEGRATION_TESTING_ENABLED=true in a production .env booted cleanly, and the
# comment made that look like a configuration the platform would have stopped.
#
# What the flag turns on is not a diagnostic: `api/integration_testing.py`
# mounts an unauthenticated forwarding ingress (`/{alias}/{target_path:path}`,
# no auth dependency by design) that is also outside the A2A rate limiter,
# since that middleware is scoped to the /a2a-rpc sub-app. In a protected
# environment that is an open relay.
#
# Same protected-environment rule as the A2A bypass above: anything not clearly
# "development" fails safe.
if settings.integration_testing_enabled and _env_is_protected():
    raise RuntimeError(
        f"INTEGRATION_TESTING_ENABLED=true with APP_ENV={settings.app_env!r}. "
        "This exposes an UNAUTHENTICATED HTTP forwarding ingress at "
        "/integration-testing/{alias}/{path} which is not covered by the A2A "
        "rate limiter. It is a development-only facility.\n"
        "  * Deploying for real: unset INTEGRATION_TESTING_ENABLED.\n"
        "  * Local development: set APP_ENV=development."
    )

# ── Fail-fast on cleartext service URLs in any environment ──
# authority_platform_url, partner_public_url and ollama_url all default to
...
_http_defaults = {
    "authority_platform_url": settings.authority_platform_url,
    "partner_public_url": settings.partner_public_url,
    "ollama_url": settings.ollama_url,
    # Both of these carry credentials and were outside the guard.
    #
    # `ainxt_base_url` is the LLM gateway base: every call ships
    # `Authorization: Bearer <PARTNER_AINXT_API_KEY>` plus the full prompt,
    # which by construction contains PARTNER.md, retrieved repository source
    # and change documents. `AINXT_BASE_URL=http://gateway.internal/v1` booted
    # in production with neither an error nor a warning.
    #
    # `partner_gitlab_url` is the fallback repo base used when a CodeRepo row
    # carries none; `rag.code_ingestion` sends the `api`-scoped PAT to it as a
    # `PRIVATE-TOKEN` header. It defaults to https://, so this only catches an
    # operator who overrode it downward — which is precisely the case that
    # needs catching.
    "ainxt_base_url": settings.ainxt_base_url,
    "partner_gitlab_url": settings.partner_gitlab_url,
}
