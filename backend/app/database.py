...
engine = create_engine(
    ...
    # Pool sizing externalized to Settings rather than hardcoded as
    # infrastructure values here. pool_recycle proactively
...
def init_db():
    ...
    _ensure_incoming_changes_authority_counter_history_column()
    _ensure_outgoing_queries_negotiation_columns()
    _ensure_incoming_changes_negotiation_columns()
    _ensure_incoming_changes_authority_followups_column()
    ...
    _ensure_partner_users_password_changed_at_column()
    ...
    # RAG / vector foundation. Extension first, then the shared
...
def _ensure_agent_jobs_observability_columns() -> None:
    ...
    remediation: `tokens_used` (per-change LLM budget accounting),
    `error_category`/`error_code` (error taxonomy), and
    `correlation_id` (causal-chain tracing).
...
def _ensure_partner_users_password_changed_at_column() -> None:
    """Add the session cut-off used to retire pre-rotation JWTs.

    NULL is the correct value for existing rows and means "no cut-off": those
    users' current sessions keep working until they next change a password.
    Backfilling `now()` instead would log every operator out on deploy for no
    security benefit — nothing about an upgrade says their sessions are
    suspect.
    """
    with engine.begin() as conn:
        conn.exec_driver_sql(
            "ALTER TABLE partner_users ADD COLUMN IF NOT EXISTS password_changed_at TIMESTAMP"
        )
...
def _ensure_code_merge_requests_project_path_column() -> None:
    """Backfill `project_path` on `code_merge_requests` (the MR link).

    The dashboard used to receive the fully-built `mr_url` and render it in an
    <a href> — a server-controlled URL flowing straight into a link target. The
    browser now rebuilds the link from `project_path` + `mr_iid`, so the server
    never hands it a URL string.
...
def _ensure_outbound_retries_idempotency_key_column() -> None:
    """Backfill `idempotency_key` on `outbound_a2a_retries` (idempotent
    retries).
...
def _ensure_change_documents_xlsx_columns() -> None:
    ...
    we store the exact authority-rendered workbook so the partner ships
    the same bytes the authority's own download endpoint serves.
    """
    with engine.begin() as conn:
        conn.exec_driver_sql(
...
def _ensure_change_documents_zip_columns() -> None:
    ...
    the partner serves the exact zip the authority built from the frozen
...
def _ensure_incoming_changes_decision_column() -> None:
    ...
    authority_counter, counter_decisions, blockers store JSON blobs (as TEXT
    ...
    with engine.begin() as conn:
        ...
        conn.exec_driver_sql(
            "ALTER TABLE incoming_changes ADD COLUMN IF NOT EXISTS authority_counter TEXT"
        )
        conn.exec_driver_sql(
...
def _ensure_incoming_changes_correlation_id_column() -> None:
    """Backfill `correlation_id` — the authority's per-(change, partner) A2A thread id,
    captured on receipt and echoed on every reply (v1.1 §5). NULL for changes
    received before this column existed / from a legacy authority that omits it."""
    with engine.begin() as conn:
        conn.exec_driver_sql(
...
def _ensure_incoming_changes_cert_status_columns() -> None:
    ...
    JSON-encoded TEXT) carries the per-run pass/fail breakdown the authority
...
def _ensure_incoming_changes_cert_signoff_columns() -> None:
    """Backfill the Certification Result certificate columns on
    `incoming_changes`.

    `cert_signoff_docx_bytes` (BYTEA) holds the .docx the authority ships on the
...
def _ensure_incoming_changes_authority_counter_history_column() -> None:
    """Backfill `incoming_changes.authority_counter_history` — append-only JSON
    list of every authority counter-proposal this partner has received, with
    each entry tagged by how the partner resolved it.

    WHY: `authority_counter` is a single-slot active-counter pointer; it gets
    ...
    with engine.begin() as conn:
        conn.exec_driver_sql(
            "ALTER TABLE incoming_changes ADD COLUMN IF NOT EXISTS authority_counter_history TEXT"
        )
...
def _ensure_outgoing_queries_response_received_at_column() -> None:
    ...
    the authority's CLARIFICATION_RESPONSE lands so the partner timeline can
...
def _ensure_incoming_changes_negotiation_columns() -> None:
    ...
    negotiation_version          — mirrors the authority's version counter (starts 1)
    negotiation_version_accepted — True = partner has accepted current version
    negotiation_finalized_at     — ISO timestamp when the authority locked the specs
    """
    with engine.begin() as conn:
        conn.exec_driver_sql(
...
def _ensure_incoming_changes_authority_followups_column() -> None:
    """Backfill `incoming_changes.authority_followups` — append-only JSON list
    of the authority's follow-up replies to an already-answered general question.
    ...
    See IncomingChange.authority_followups + handle_clarification_response.
    """
    with engine.begin() as conn:
        conn.exec_driver_sql(
            "ALTER TABLE incoming_changes ADD COLUMN IF NOT EXISTS authority_followups TEXT"
        )


def _ensure_incoming_changes_round_notices_column() -> None:
    """Backfill `incoming_changes.round_notices` — append-only JSON list of
    authority round-lifecycle notices (PM force-closed a round). Surfaced in the
...
def _ensure_incoming_changes_change_summary_column() -> None:
    """Backfill `incoming_changes.authority_change_summary` — the "summary of changes"
    the authority ships in the kit envelope (change_summary) describing what changed in
    this kit version. Surfaced as a banner on the partner change page.
    """
    with engine.begin() as conn:
        conn.exec_driver_sql(
            "ALTER TABLE incoming_changes ADD COLUMN IF NOT EXISTS authority_change_summary TEXT"
        )


def _ensure_incoming_changes_revision_hold_columns() -> None:
    """Backfill the revision-hold flags — set while the authority prepares a revised kit
...
def _ensure_incoming_changes_emergency_issues_column() -> None:
    ...
    post-freeze emergency issues raised to the authority + resolutions. See the
    emergency-issue endpoint in api/dashboard/defects.py."""
    with engine.begin() as conn:
        conn.exec_driver_sql(
...
def _ensure_outgoing_queries_correlation_id_column() -> None:
    ...
    query, echoed by the authority on the CLARIFICATION_RESPONSE so we attach the
