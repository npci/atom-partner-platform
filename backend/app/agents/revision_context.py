...
#
# BOUNDED, LRU-evicted (per the "Mechanical Sympathy and Shared-Nothing
# Concurrency" principle — "Flag when: unbounded queues are used"; "Recommend: bounded
...
def _summary_system() -> str:
    """System prompt for the revision-diff summariser.

    A function, not a constant: it reads the active pack, and a module constant
    would freeze whichever pack was live at import.
    """
    _p = get_active_pack()
    return (
    f"You compare two versions of {_p.prompt_block('authority', 'authority')} change documents "
    "and summarise WHAT CHANGED, "
    "concisely, for the partner's implementation team. Output plain-text bullet points "
...
def context_cache_stats() -> dict:
    """Observability hook — current size and capacity, so
...
def _assemble_change_context_uncached(db: Session, change_id: str, api_key: str | None = None) -> dict:
    ...
    if not changed:
        return {
            ...
            "revision_summary": (
                f"{get_active_pack().prompt_block('authority_cap', 'The authority')} published "
                f"revisions up to v{current_version}, but the documents "
...
def _summarize_changes(changed: list[dict], current_version: int, api_key: str | None) -> str:
    ...
    return (
        f"{get_active_pack().prompt_block('authority_cap', 'The authority')} revised the kit "
        f"up to v{current_version}. Changed documents: {names}. "
