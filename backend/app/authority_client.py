...
"""A2A client for communicating with the authority platform.

The unified A2A SDK refactor deleted the authority's legacy
...
This is the symmetric mirror of the authority's `app.services.a2a_client`:
auth handshake then SDK send via `app.a2a_common.client.send_a2a_message`.
HMAC envelope rides for free when `partner_settings.authority_hmac_secret`
is installed.

Auth note: we keep using the api_key→JWT handshake at /api/a2a/auth.
The authority's inbound `decode_partner_token` validates with `settings.secret_key`
(platform-wide), which only the handshake mints with. Locally minting
with `authority_jwt_secret` would NOT work — that's the per-partner secret
used in the OPPOSITE direction (authority→partner).
"""
...
# Used by run_authority_reachability_check() to prevent server-side request forgery
...
# unreachable. The authority's UAT platform lives on RFC-1918 space, so a flat "block
...
#
# TIER 1 — never a legitimate authority platform, not overridable.
...
# approved target via `authority_ssrf_allowed_hosts` (per host, preferred) or
# `authority_ssrf_allow_private_networks` (blanket). This is the tier that a
# real internal authority platform lands in.
_PRIVATE_NETS = [
...
def _ssrf_allowed_hosts() -> frozenset[str]:
    ...
    raw = getattr(settings, "authority_ssrf_allowed_hosts", "") or ""
    return frozenset(
...
def _ssrf_allow_private() -> bool:
    ...
    return bool(getattr(settings, "authority_ssrf_allow_private_networks", False))


def _configured_authority_host() -> str:
    """Hostname of the authority this platform is configured to talk to.

    Read at CALL time, like the two helpers above, so a settings change takes
    effect on the next send rather than at the next restart. Returns "" when
    unset, which matches nothing — an unconfigured platform gets no exemption.
    """
    from app.config import settings

    raw = (getattr(settings, "authority_platform_url", "") or "").strip()
    if not raw:
        return ""
...
def _is_private_url(url: str, *, unresolved: bool = True) -> bool:
    ...
    `authority_ssrf_allowed_hosts` or `authority_ssrf_allow_private_networks` is on —
    which is how an internal authority platform, the normal deployment shape, is
    ...
    try:
        ...
        # Compose-shape exemption. These three names are HARDCODED, so an
        # operator cannot aim them anywhere: the blast radius is "the platform
        # probes itself or a sibling container", which is what the compose
        # setup needs and what the names describe. They short-circuit ahead of
        # resolution because under compose they resolve to addresses this guard
        # would otherwise refuse.
        if host_l in ("localhost", "host.docker.internal") or host_l.endswith("_backend"):
            return False

        # The CONFIGURED authority host is exempt from the PRIVATE-space rule,
        # so a host install pointed at an RFC-1918 authority is reachable
        # without the operator having to find AUTHORITY_SSRF_ALLOWED_HOSTS
        # first — the symptom of that discovery problem reads as the authority
        # being down rather than as a guard decision.
        #
        # It waives tier 2 ONLY, and it does so by joining `allow_private`
        # rather than returning early. That distinction is the whole control:
        # an early return would skip the resolution below and with it the
        # tier-1 checks, so `AUTHORITY_PLATFORM_URL=http://169.254.169.254/`
        # would make the metadata service reachable by configuration — exactly
        # what `_NEVER_ALLOWED_NETS` promises is not overridable. Falling
        # through means the configured host is still resolved and still
        # refused if it lands in loopback or link-local space.
        allowlisted = host_l in _ssrf_allowed_hosts()
        is_configured_authority = bool(host_l) and host_l == _configured_authority_host()
        allow_private = allowlisted or is_configured_authority or _ssrf_allow_private()
        ...
        for family, _type, _proto, _cname, sockaddr in addrs:
            ...
            for net in _PRIVATE_NETS:
                if ip.version == net.version and ip in net:
                    if allow_private:
                        if allowlisted:
                            approved_via = "AUTHORITY_SSRF_ALLOWED_HOSTS"
                        elif is_configured_authority:
                            approved_via = "AUTHORITY_PLATFORM_URL (configured authority)"
                        else:
                            approved_via = "AUTHORITY_SSRF_ALLOW_PRIVATE_NETWORKS"
                        logger.info(
                            "SSRF guard: permitting private IP %s (host=%s) — approved via %s",
                            ip, host, approved_via,
                        )
                        continue
                    logger.warning(
                        "SSRF guard: blocked connection to private IP %s (host=%s) — add the host "
                        "to AUTHORITY_SSRF_ALLOWED_HOSTS if this is the real authority platform", ip, host,
                    )
                    return True
        return False
...
def _guard_outbound_url(url: str, purpose: str) -> None:
    ...
    `run_authority_reachability_check()` — the Settings page's "Test Connection"
    ...
    if _is_private_url(url, unresolved=False):
        raise OutboundURLBlocked(
            ...
            "authority platform, approve it by adding the host to "
            "AUTHORITY_SSRF_ALLOWED_HOSTS, or set "
...
def _get_setting(db: Session, key: str, default: str = "") -> str:
    ...
    in `core.secret_box.SECRET_KEYS`;
    ...
    try:
        ...
        # (keeps sensitive material out of the logs).
        logger.critical(
...
def _get_a2a_base_url(db: Session) -> str:
    """Direct service URL for partner→authority A2A traffic.

    Defaults to the docker service name. Override via
    `partner_settings.authority_a2a_url`. Distinct from `authority_platform_url`
    ...
    """
    url = _get_setting(db, "authority_a2a_url", "http://authority_backend:8000").strip().rstrip("/")
    return url


def _get_human_authority_url(db: Session) -> str:
    """UI-facing authority URL (used by `run_authority_reachability_check`)."""
    url = _get_setting(db, "authority_platform_url", "http://localhost").strip().rstrip("/")
    if "://localhost" in url:
...
def authenticate(db: Session) -> str | None:
    """Authenticate with the authority and return a JWT token.
    ...
    _validate_url_scheme(url, purpose="authority A2A auth (API key transmission)")
    ...
    _guard_outbound_url(url, "authority A2A auth (API key transmission)")
    api_key = _get_api_key(db)
    if not api_key:
        logger.error("Authority API key not configured")
        return None
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.info("Authority auth success")
            return token
        else:
            # Status only — never the body. The request that produced it POSTs
            # `{"api_key": <the partner API key>}`, and the authority is also a
            # FastAPI service: a Pydantic 422 renders the rejected input back,
            # so `resp.text` could echo the key itself into this platform's
            # logs at WARNING, which is on by default. The AST guard in
            # test_no_raw_secret_key_logging does not see this because
            # `resp.text` is not one of the secret-bearing attributes it knows.
            logger.warning("Authority auth failed: HTTP %d", resp.status_code)
            return None
    except Exception as e:
        logger.error("Authority auth error", exc_info=True)
        return None
...
def _resolve_correlation_id(db: Session, change_id: str | None, payload: dict) -> str | None:
    ...
    echo the authority's per-(change, partner) thread id captured on receipt, so every
    ...
    if not corr and change_id:
        row = (
            ...
            .filter(IncomingChange.authority_change_id == change_id)
            .first()
        )
...
async def _dispatch_wire(
    ...
    *, job_correlation_id: str | None = None, idempotency_key: str | None = None,
    ...
    send. Returns the receiver's structured reply (the authority's task receipt; since
    ITA-2 possibly carrying a merged handler dict) or None when no structured
    artifact came back.

    Two DISTINCT correlation concepts are in play here, and must not be
    conflated:
      - `envelope_corr` — the A2A protocol's own business correlation_id
        (v1.1 §5): the authority's per-(change, partner) CONVERSATION THREAD pointer.
        This has spec meaning to the authority's routing and is resolved exactly as
      ...
      - `job_correlation_id` — this platform's OWN causal-chain id (an
        ...
        independent of and never overriding the authority conversation thread.
    """
    token = authenticate(db)
    if not token:
        raise RuntimeError("Authority authentication failed (no API key configured, or the authority rejected it)")

    base_url = _get_a2a_base_url(db)
    _validate_url_scheme(base_url, purpose="authority A2A task send (Bearer JWT + HMAC)")
    ...
    _guard_outbound_url(base_url, "authority A2A task send (Bearer JWT + HMAC)")
    hmac_secret = _get_setting(db, "authority_hmac_secret") or None

    # Protocol v1: envelope carries protocol_version + message_id
    # (dedup key) + correlation_id + timestamp. Reuse message_id as the SDK
    # task_id so the wire dedup key and the SDK Task id stay 1:1.
    #
    # IDEMPOTENCY (idempotent operations and safe retries — the classic
    # failure being non-idempotent payment processing with retry paths).
    ...
    # behaviour) meant a send that the authority actually processed but whose ACK was
...
async def send_task_async(
    ...
    *, correlation_id: str | None = None, timeout: float | None = None,
    ...
    breaker = breaker_for("authority_a2a_outbound")
    bulkhead = bulkhead_for("authority_a2a_outbound")
    ...
    try:
        ...
        logger.error(
            "A2A send rejected: circuit open for authority outbound (type=%s change=%s)",
            task_type, change_id,
        )
        ...
        # resolved authority host, port and token prefix into a UI-visible row
    ...
    logger.info("A2A task sent to the authority: type=%s change=%s", task_type, change_id)
...
def send_task(
    ...
    *, correlation_id: str | None = None, timeout: float | None = None,
    ...
    """Send an A2A Task to the authority via the SDK JSON-RPC mount.

    Resilience: wrapped in a circuit breaker + bulkhead (applied to the
    `authority_a2a_outbound` boundary) so a down/degraded authority
    ...
    `correlation_id` lets a caller explicitly set the platform's
    ...
    Returns:
        The receiver's structured reply body on success (B2 — the authority's task
...
def _build_query_payload(message: str, correlation_id: str | None, phase: str | None) -> dict:
    ...
    `bank_lead`) are null/empty pending UI capture — never fabricated.
    `priority` defaults to `"normal"`. Legacy `message` is retained: the authority's
    ...
    payload: dict = {
        ...
        "clause_ref":          None,
        "subject":             None,
        # Dual-written, neutral key first — see `send_blocker` for the rationale
        # and the removal condition. All of these are `None` pending UI capture,
        # so only the KEY NAME travels today; that is precisely what made them
        # easy to miss.
        "partner_interpretation": None,
        ...
        "partner_lead":        None,
        "bank_lead":           None,
        # back-compat — the authority reads `message`; `phase` drives cert-vs-general routing.
        "message":             message,
    }
    if correlation_id:
...
def send_query(
    ...
    """Send a general-channel clarification query to the authority.

    `correlation_id` is the partner-minted UUID of the OutgoingQuery row
    we're about to persist. The authority echoes it back on the CLARIFICATION_RESPONSE
...
def send_cert_query(
    ...
    with payload `phase='cert'` (was a dedicated cert_query task type); the authority's
...
def send_cert_status_update(
    ...
    """Push a cert lifecycle status update to the authority via A2A.
    ...
    so the authority's cert orchestrator can pre-configure cert-agent test
    cases before triggering the LLM cert run.

    `test_data_per_case` is the per-TC override dict the
    partner UI builds from its ChangeTestData rows. Shape is
    `{tc_id: {field: value, ...}}`. Per-case values take priority over
    the flat `test_data` fallback on the authority side.
    """
...
def report_progress(db: Session, change_id: str, step: str, notes: str = "") -> dict | None:
    """Report an implementation milestone to the authority (protocol v1 `milestone_update`).
    ...
    `completed_at` and `next_milestone` are populated for real (the authority already
...
def declare_ready(
    ...
    """Send CERT_READINESS_DECLARATION to the authority with the implementing role.

    The authority's cert orchestrator uses `role` (PAYER_PSP / PAYEE_PSP /
    ...
    `test_data_per_case` carries per-TC overrides keyed by
    tc_id. The authority merges these on top of the flat `test_data` fallback
    when patching cert-agent rows.

    Empty role/test_data degrades to the legacy `{status: ready_for_cert}`
    payload — back-compat with authority sides that haven't deployed the
    orchestrator yet.
    ...
    authority's cert orchestrator reads them off this message, so they're kept (a
...
def send_proposal_acceptance(
    ...
    the authority persists onto `assignment.acceptance_meta.accepted`.
    ...
    (permissive-payload) partner enrichments the authority already reads and persists.
    """
    from datetime import datetime, timezone

    # Version being accepted — the current kit version the partner holds.
    row = (
        db.query(IncomingChange)
        .filter(IncomingChange.authority_change_id == change_id)
        .first()
...
def send_counter_proposal(
    ...
    `counter_proposal` task type so the authority routes to the structured
    ...
    payload = {
        ...
        "references":           [],
        # Bilateral-extension / back-compat fields (kept; the authority's handler reads these):
        "decision":             "COUNTER",
...
def send_counter_decision(
    ...
    """Sent when partner accepts (or rejects) an authority-originated counter
    in isolation — i.e. resolving just the negotiation thread without
    accepting the whole rollout. Mirror of the authority→partner
    COUNTER_DECISION; uses a dedicated `counter_decision` task type so
    the authority's executor routes it to the structured handler.
    """
    payload = {
...
def send_blocker(
    ...
    requested_action_from_authority: str | None = None,
) -> dict | None:
    """Sent when partner reports an obstacle mid-implementation
    (Journey C of the rollout doc). Structured fields let the authority's PM
    ...
    downgrade). The authority's receiver stores the whole payload, so new fields persist.
    """
    from datetime import datetime, timezone
    payload = {
        ...
        # DUAL-WRITTEN. The neutral key is the one to read; the branded one is
        # still sent because the authority's receiver reads it today and the two
        # repos deploy separately — dropping it here would silently blank the
        # field on the authority's side of a live blocker.
        #
        # This is the outbound mirror of the inbound dual-accept the authority
        # already does for `SENDER_AUTHORITY` / `X-Auth-*`: emit both, let the
        # counterparty switch reads at its own pace, then drop the old key.
        # REMOVE `requested_action_from_npci`, `bank_internal_ticket` and
        # `bank_lead` once the authority reads the neutral spellings.
        "requested_action_from_authority": requested_action_from_authority,
        "requested_action_from_npci":      requested_action_from_authority,
        "blocks":                    [],
        "partner_internal_ticket":   None,
...
# ── Certification lifecycle senders (protocol v1, Partner→Authority) ──────────


def send_cert_config_submission(db: Session, change_id: str, config: dict) -> dict | None:
    """Submit the partner's cert config in response to cert_config_request (§7.2)."""
...
def send_cert_verdict_dispute(
    ...
    """Dispute an authority cert verdict (§7.7)."""
    return send_task(
...
def send_emergency_issue(
    ...
    the authority freezes the change, queries/counters are rejected — this is the only
    inbound task the authority still accepts. Wire format = task_type='emergency_issue'.
    """
    payload = {
...
# from `secret_box.safe_key_label()` rather than by the raw `authority_hmac_secret`
# settings key, because static analysis repeatedly flagged the earlier inline
# form as a hardcoded password in a connection string. A string literal that
# contains a known secret identifier and is returned from a function reachable
# by an HTTP handler matches that pattern, regardless of the literal being
...
_HMAC_SECRET_LABEL = safe_key_label("authority_hmac_secret")

_HMAC_REJECTED_MSG = (
    f"HMAC envelope rejected (HTTP 401) — verify {_HMAC_SECRET_LABEL} in Settings "
    "matches the value the authority has stored for this partner."
)
...
#
# The authority answers EVERY rejection on /a2a-rpc/rpc with 401. Its JWT layer
...
_CLOCK_SKEW_MSG = (
    ...
    f"more than {DEFAULT_MAX_SKEW_S}s from the authority's, so the signed timestamp is "
    "outside the accepted window. No secret is wrong: fix the clock (NTP) and "
    "retry."
)

_HMAC_SECRET_UNKNOWN_TO_AUTHORITY_MSG = (
    "HMAC envelope rejected (HTTP 401, hmac_secret_not_configured) — this "
    "platform signed the request but the authority holds NO signing secret for this "
    f"partner. {_HMAC_SECRET_LABEL} must be installed on the authority side too "
...
def _describe_rejection(exc: BaseException) -> str | None:
    ...
    if code == "hmac_secret_not_configured":
        return _HMAC_SECRET_UNKNOWN_TO_AUTHORITY_MSG
    ...
    if code in _HMAC_ERROR_CODES:
        return (f"HMAC envelope rejected (HTTP {status}, {code}) — the signed "
                "envelope did not verify at the authority.")
    if code in _JWT_ERROR_CODES:
        return f"{_JWT_REJECTED_MSG}: {code}"
    if status == 404:
        return "/a2a-rpc/rpc not found (HTTP 404) — the authority's SDK mount may be disabled"
    if code:
        return f"The authority rejected the echo probe (HTTP {status}, {code})"
    return None
...
def _send_echo_probe(
    ...
    The authority's executor exercises auth middleware (Bearer JWT) → HMAC
    ...
    try:
        ...
        # response body, not by the status code: the authority answers both a bad JWT
        ...
        # (DNS, TLS, connection refused, timeout) or the authority answered with
        # something unstructured. The raw message is the best signal left.
        msg = str(exc)
        if "404" in msg:
            return False, "/a2a-rpc/rpc not found (HTTP 404) — the authority's SDK mount may be disabled"
        return False, msg[:200]


def run_authority_reachability_check(
    db: Session, api_key_override: str | None = None,
) -> dict:
    """Probe connectivity AND the end-to-end A2A round-trip against the authority.

    Named `run_authority_reachability_check` rather than `test_connection` for two
    reasons. It is an outbound HTTPS probe of the authority platform, not a database
    ...
    credentials, which is the shape static analysis treats as a
    database-connect sink; every operator diagnostic returned from here was
    reported as a leaked connection password.
    ...
    Three layered checks so the UI button is actually diagnostic:
      1. Reachability — GET the public agent card at authority_platform_url.
         Catches: the authority down, wrong URL.
      ...
      3. Echo round-trip — send task_type='echo' via the SDK to
         ...
         authority_hmac_secret, A2A mount disabled, JWT-layer rejection.
    ...
    """
    human_url = _get_human_authority_url(db)
    ...
    # only "point at the public authority platform endpoint", which sent operators
    # hunting for a public address that does not exist — the authority's UAT is an
    # internal host, and approving it is the correct resolution.
    _allow_hint = (
        "If this IS the real authority platform, approve it by adding the host to "
        "AUTHORITY_SSRF_ALLOWED_HOSTS (comma-separated), or set "
        "AUTHORITY_SSRF_ALLOW_PRIVATE_NETWORKS=true to permit private space wholesale."
    )
    ...
    _validate_url_scheme(human_url, purpose="authority platform URL (reachability check)")
    _validate_url_scheme(a2a_url, purpose="authority A2A URL (auth + SDK round-trip)")

    # Step 1 — reachability via the public well-known endpoint.
    authority_name = "Authority Platform"
    try:
        ...
        try:
            authority_name = resp.json().get("name") or authority_name
        except Exception:  # noqa: BLE001
            pass
    except httpx.ConnectError:
        return {"status": "error", "message": f"Cannot reach the authority at {human_url}"}
    ...
    if not api_key:
        return {
            ...
            "message": "Reachable, but Partner API Key is not configured. "
                       "Paste the key from the authority's admin console into Settings before any "
    ...
    if auth_resp.status_code in (401, 403):
        return {
            "status": "error",
            "message": f"The authority rejected the API key (HTTP {auth_resp.status_code}). "
                       "Rotate the key on the authority's admin console and re-install in Settings, or verify "
                       "this partner row is still Active on the authority.",
        }
    ...
    if not hmac_secret:
        return {
            "status": "ok",
            "message": f"Connected to {authority_name} — reachability + API key "
    ...
    return {
        "status": "ok",
        "message": f"Connected to {authority_name} — reachability + API key + full A2A round-trip verified",
    }
