...
Part of the A2A security hardening — partner-side mirror of NPCI's
`backend/app/a2a_common/sdk_hmac_middleware.py`. Lighter:
  ...
  * No partner-id lookup. The partner stack is the receiver of exactly
    ...
    `partner_settings.authority_hmac_secret` (one row, encrypted at rest via
    core/secret_box.py).
...
FAIL-CLOSED DEFAULT: when no
`authority_hmac_secret` is configured, inbound requests are REJECTED (503), not
...
# is not `_SETTING_NAME = "<literal>"`, the shape a "Use Of Hardcoded Password"
# static-analysis rule reported here. Equal to the plain string in every
# respect; the DB lookup is unchanged.
_SETTING_NAME = SettingKey.authority_hmac_secret
...
# from that secret, and reports the subsequent log call as leaking it — a
# sensitive-log-filtering defect. Mapping the code through this frozenset severs
...
class PartnerHmacMiddleware:
    ...
    def __init__(
        ...
        *,
        ...
        self._nonce_store_cached = None

    def _nonce_store(self):
        """The replay-protection store, built once on first use.

        Lazy rather than built in __init__ because the middleware is
        constructed at import time, before the database engine is configured —
        and because a test that swaps SessionLocal must be able to do so
        before the first request rather than before the import.
        """
        if self._nonce_store_cached is None:
            from .nonce_store_db import build_nonce_store
            self._nonce_store_cached = build_nonce_store()
        return self._nonce_store_cached
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # Bounded body read — smallest practical size limit, enforced
        ...
        if not secret:
            if allow_unconfigured_bypass():
                if not self._warned_no_secret:
                    # safe_key_label(), not _SETTING_NAME — see the note on
                    # sensitive-log filtering in _load_secret().
            ...
            await _send_json(send, 503, {
                ...
                "detail": (
                    "The authority HMAC secret is not configured on this partner "
                    "instance. Inbound A2A calls are rejected until an operator "
                    "installs authority_hmac_secret in Settings. This is a "
                    "fail-closed default."
                ),
            })
            return
        ...
        # A real nonce store, not None. Passing None made `hmac_signer.verify`
        # SKIP the uniqueness check entirely, so `"replay_detected"` was
        # unreachable on this platform and a captured, correctly-signed request
        # could be replayed for the whole 300s skew window — long enough to
        # flip cert_status to "certified" or re-fire a credentialed trigger per
        # case id. Redis when configured, Postgres otherwise; see
        # nonce_store_db.build_nonce_store.
        ok, err = hmac_verify(
            envelope_headers, body, secret,
            redis_client=self._nonce_store(),
    ...
    def _load_secret(self) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                # secret_box.safe_key_label(). A variable holding the key NAME
                # is indistinguishable from one holding its VALUE to a taint
                # engine.
                logger.critical(
...
async def _read_body(receive: Receive, *, max_bytes: int) -> bytes:
    ...
    after the body is fully buffered — the smallest practical size limit,
    applied as an inbound request control on an H3 interface."""
    chunks: list[bytes] = []
    total = 0
    while True:
        msg = await receive()
