...
`authority_client.send_task()` on a transient failure or open circuit breaker.

Implements the platform's "DLQ and replay process" requirement for outbound
delivery.

Exponential backoff: 1 -> 5 -> 15 -> 60 minutes between attempts (mirrors the
authority-side outbound delivery backoff already documented in the codebase's own
`a2a_client.py`), capped at `max_attempts`. A row that
...
def enqueue(
    ...
    *,
    ...
    authority_client.send_task() on any transport failure (including a
    ...
    Persisting it is what makes the retry safe: every later
...
def run_sweep(db: Session, *, max_attempts: int | None = None) -> dict:
    ...
    authority; this background sweep already paces itself via the backoff
    schedule and should make a real attempt each time a row comes due,
    independent of the interactive path's breaker state."""
    # Import here (not at module top) to avoid a circular import — authority_client
    # imports this module, and this module needs authority_client's send helper.
    ...
    from app.authority_client import _dispatch_wire, _run_portably
    ...
    for row in due:
        ...
        row.attempts += 1
        try:
            # Re-send under the ORIGINAL envelope message_id so the authority can
            # deduplicate an attempt it already processed.
            ...
            # Type only — matches the enqueue path in authority_client.send_task;
            ...
            if row.attempts >= max_attempts:
                ...
                emit_security_event(
                    ...
                    severity="high",
                    boundary="authority_a2a_outbound",
                    decision="abandoned",
