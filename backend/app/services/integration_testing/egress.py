...
def perform_exchange(payload: dict) -> dict:
    ...
    try:
        ...
        # Type only. This `detail` travels back over A2A and is re-emitted as
        # an HTTP response body by api/integration_testing, so `{exc}` handed
        # the remote peer httpx's text for a failure against an INTERNAL
        # target — resolved host, port, and TLS specifics. The type was
        # already being computed alongside it, so the raw message added
        # nothing the caller could act on.
        logger.warning(
            "tunnel egress failed alias=%s exc=%s", decoded.alias, type(exc).__name__)
        logger.debug("tunnel egress failure detail", exc_info=True)
        return encode_error(exchange_id=exchange_id, code=ErrorCode.TARGET_UNREACHABLE,
                            detail=f"{decoded.alias}: {type(exc).__name__}")
    finally:
...
# async handler that reaches today's `authority_client` hits `asyncio.run` inside a
