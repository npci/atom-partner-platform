...
The REVERSE direction's near end. The External API (or anything on the partner's
...
typically the Simulator's callback API. Mirror of the authority repo's
...
(no partner parameter), sends through `authority_client.send_task_async` with the
...
async def forward_exchange(
    *,
    ...
    from app.authority_client import send_task_async
