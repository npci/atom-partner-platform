...
The REVERSE direction's front door: the External API (or anything partner-side
...
against its own allowlist. Mirror of the authority repo's
...
def _rig_authorised(request: Request, db: Session) -> Response | None:
    ...
    # Encrypted at rest as of the SECRET_SETTING_KEYS fix — it was the one live
    # credential stored in cleartext. `decrypt()` returns an untagged value
    # unchanged, so a row written before that change still reads correctly and
    # no migration has to run before this endpoint works.
    expected = decrypt((row.value if row else "") or "").strip()
    if not expected:
        return Response(
            status_code=503,
...
def report_cert_case_outcome(body: dict, request: Request,
    ...
    # DUAL-ACCEPT, and not optional. This body comes from the certification RIG
    # — a separately built and separately deployed artifact
    # (`adcn-operator-app/cert-rig`), which still posts `npci_change_id`.
    # Accepting only the new spelling makes every case outcome fail the 422
    # below, so the rig executes, the SUT answers, and not one result is ever
    # recorded. Drop the fallback once the rig emits the new key.
    authority_change_id = str(
        body.get("authority_change_id") or body.get("npci_change_id") or ""
    ).strip()
    case_id = str(body.get("case_id") or "").strip()
    status = str(body.get("status") or "").strip().lower()
    attempt = int(body.get("cert_attempt") or 1)
    details = body.get("details") if isinstance(body.get("details"), dict) else {}
    if not authority_change_id or not case_id or status not in ("passed", "failed", "error"):
        return Response(status_code=422,
                        content=b'{"error":"authority_change_id, case_id and '
    ...
    db.add(CertCaseExecution(
        authority_change_id=authority_change_id, case_id=case_id, cert_attempt=attempt,
        cflow_id=(details.get("cert_context") or {}).get("cflow_id"),
        status=status, details=details,
    ))
    db.commit()

    from app.authority_client import send_cert_case_result

    reply = send_cert_case_result(db, authority_change_id, case_id, status,
...
def list_cert_executions(authority_change_id: str, limit: int = 200,
    ...
    rows = (db.query(CertCaseExecution)
            .filter(CertCaseExecution.authority_change_id == authority_change_id)
