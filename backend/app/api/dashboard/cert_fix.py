...
`authority_client.send_cert_fix_notification` — the gate the whole loop's safety
rests on. It refuses any round not at `awaiting_approval`, and a failed send
leaves the round parked: telling the partner the authority was notified when it
...
def approve_and_notify(change_id: str, round_id: str,
    ...
    from app.authority_client import send_cert_fix_notification
