...
def start_job(
    ...
    change has already exhausted its per-change LLM token budget — checked
    synchronously, BEFORE the 202 is returned, so a non-converging loop is
    stopped at the API boundary rather than discovered after another
    background job has already spent
    more tokens. 503 while the platform is draining for shutdown, or when the
    cross-change concurrency cap is saturated.
...
def _run_job(job_id: str, runner: JobRunner) -> None:
    ...
    Wraps the runner in TWO context managers:
      - `core.llm.track_token_usage()` — sums every call_llm() invocation the
        ...
        agents/code_files.py's batched generation) onto the job row.
      - `core.correlation.use_correlation_id(job.correlation_id)` — makes this
        job's correlation id the default for any `authority_client.send_task()`
        call the runner triggers, without threading it through every runner's
        signature.

    And enforces two process-level guarantees:
    ...
    # a connection-pool bottleneck, and the pool sizing assumes
