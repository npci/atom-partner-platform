...
class IncomingChange(Base):
    ...
    authority_change_id: Mapped[str] = mapped_column(String(36), nullable=False)
    # The authority's per-(change, partner) A2A thread correlation_id, captured from the
    # inbound change_communication envelope. Echoed back on every reply about
    # this change (v1.1 §5) so the authority threads our replies to the right conversation.
    ...
    # Latest authority counter-proposal received (multi-round negotiation).
    # Stored as JSON: {counter_proposal_id, negotiation_round, justification,
    # valid_until, received_at, status:'open'|'responded'}. NULL until the authority
    # sends their first counter; cleared back to NULL when partner responds.
    authority_counter: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Append-only audit log of every authority counter-proposal this partner
    # has received, with how it was resolved. Each entry is the prior
    # `authority_counter` snapshot at clear-time plus { resolved_at,
    ...
    authority_counter_history: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Append-only log of authority decisions on partner counters: each entry
    ...
    # Append-only list of blockers we've reported to the authority + any
    ...
    # resolution the authority sent back. Each entry: {issue_id, severity, title,
    ...
    # arrives via change_communication; the partner user advances through the
    # remaining three from the partner UI. Each transition fires an A2A
    # CERT_STATUS_UPDATE to the authority via the SDK.
    ...
    # Latest cert_test_response payload from the authority — JSON-encoded.
    ...
    # Mirrors ChangeRequest.negotiation_version on the authority side.
    # Set to 1 on receipt; bumped when the authority sends a new version.
    ...
    # When the authority finalised the negotiation (specs locked). NULL = still open.
    negotiation_finalized_at: Mapped[str | None] = mapped_column(Text, nullable=True)
    # The authority's "summary of changes" shipped in the kit envelope (change_summary):
    # what changed in this kit version. Shown as a banner on the change page.
    authority_change_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    # True while the authority is preparing a revised kit (round closed → new version
    ...
    # The version the authority is building (N+1), for the hold banner text.
    ...
    # further replies the authority sends to the SAME question are appended here so
    # the timeline shows every reply (mirroring how the authority keeps each
    ...
    authority_followups: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Append-only log of authority round-lifecycle notices (currently: PM
    ...
    # Certification Result certificate (.docx) shipped on the all-PASS
    # cert_completion_signoff A2A task. Stored verbatim (BLOB) so the partner
    # serves the exact bytes the authority signed, with the filename for the download
...
class ChangeDocument(Base):
    ...
    # Which published kit version (mirrors the authority's ChangeRequest.negotiation_version)
    ...
    negotiation_version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    # When the authority side has a `.docx` rendition for this doc (docgen
    ...
    # offer a download without bouncing back to the authority.
    ...
    # cert_test_cases — companion .xlsx rendered by the authority's
    # excel_testcase_engine. Stored verbatim so the partner serves
    # the exact authority workbook (Index / Summary / Modes / archetype
    ...
    # verbatim so the partner UI can play + download it without bouncing to the authority.
    ...
    # XSD multi-schema bundle — when the authority's change touches ≥2 .xsd files it ships
...
class OutgoingQuery(Base):
    ...
    # Partner-minted UUID per query; echoed by the authority on the matching
    ...
    # When the authority's CLARIFICATION_RESPONSE landed on this row. NULL until
...
class ChangeTestData(Base):
    ...
    The partner UI shows one form per authority-initiated TC discovered from
    the cert_test_cases ChangeDocument. Partner-initiated TCs do not need
    test data here — the partner runs them on its own simulator side and
    ...
    without a migration per new field. The orchestrator on the authority side
...
class PartnerUser(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime, default=_now, nullable=False)
    # Session cut-off. Every JWT carries an `iat`; `api.auth.get_current_user`
    # rejects a token issued at or before this instant, so changing a password
    # invalidates the sessions that existed under the old one.
    #
    # Without it, password rotation — the standard containment step after a
    # cookie is stolen — did nothing: the thief kept access for the remaining
    # 24h of the token's life, and the admin-driven reset in `api/users.py`
    # was no better. Nullable so existing rows need no backfill; NULL means
    # "no cut-off", i.e. the pre-existing behaviour.
    password_changed_at: Mapped[datetime] = mapped_column(DateTime, nullable=True)


class A2ANonce(Base):
    """Seen-nonce record for A2A replay protection.

    `hmac_signer.verify()` skips the uniqueness check entirely when handed no
    nonce store, and the middleware passed None — so the only bound on
    replaying a captured signed request was the 300s timestamp skew. See
    `a2a_common/nonce_store_db.py` for why this lives in Postgres rather than
    requiring every partner fork to run redis.

    The PRIMARY KEY is the control: uniqueness is decided by the database, so
    two concurrent replays cannot both be admitted by a read-then-write race.
    Rows are swept opportunistically once past `expires_at`.
    """
    __tablename__ = "a2a_nonces"
...
class AgentRun(Base):
    ...
    authority backend's `agent_jobs` table.
    ...
    partner ops user + the authority a queryable record of which agent ran, against
...
class FeasibilityReport(Base):
    """Structured per-area feasibility assessment of an incoming authority change.
    ...
    # match the partner-side convention (authority_counter, cert_summary, etc.).
    content: Mapped[str] = mapped_column(Text, nullable=False)
...
class DesignReport(Base):
    """Partner-side design document for an incoming authority change.
...
class TestReport(Base):
    """Partner-side test plan for an incoming authority change.

    Produced by `app.agents.testing` (TestAgent) against PARTNER.md + the change's
    documents (incl. authority cert_test_cases) + the design doc. Authoring only — the
    authoritative cert run is delegated to the authority's orchestrator via the existing
...
class CodeReport(Base):
    """Partner-side implementation plan for an incoming authority change.

    Produced by `app.agents.code` (CodeAgent). MVP is spec-grounded only — built
    from the design doc + authority docs + PARTNER.md, NOT the partner's source code
...
class KnowledgeDoc(Base):
    ...
    The source of truth for an uploaded KB doc (network specs, authority circulars,
...
class CodeRepo(Base):
    """A partner GitLab repository registered for the Code RAG.
    ...
    gitlab_repo: Mapped[str] = mapped_column(String(500), nullable=False)        # e.g. "group/service-stack"
...
class CodeMergeRequest(Base):
    ...
    # to the browser — see _mr_view() in api/dashboard/code.py. A server-supplied
    # URL flowing from the API response into an <a href> is a Client DOM XSS /
    # Reflected XSS / Open Redirect sink. The UI now rebuilds the link
...
class AgentJob(Base):
    ...
    # Structured classification of `error` (the error taxonomy) — additive
    ...
    # Feeds the per-change budget guard (core/llm_budget.py) — cost is a
    # bounded resource. NULL for jobs that predate token tracking.
    tokens_used: Mapped[int | None] = mapped_column(Integer, nullable=True)
    # One correlation id per job, auto-generated at creation, threaded through
    # to every outbound A2A call the job's runner triggers (authority_client.py's
    # `correlation_id=` parameter) and available for LLM/GitLab call sites to
    # pick up too (correlation IDs MUST propagate across sync calls, async
    # messages, internal modules, and dependency calls). Distinct from the A2A
    # envelope's own `correlation_id` field (the authority thread pointer) — this
    # is the platform's OWN causal-chain id for "why did this specific
    # outbound send/LLM call happen," independent of which authority conversation
    # thread it rides on.
...
class GeneratedCodeFile(Base):
    ...
    # (nothing to fix yet — the initial generation). Gives the platform a
    # persisted, queryable audit trail from finding -> fix, distinct from the UI's
...
class OutboundA2ARetry(Base):
    """A failed `authority_client.send_task()` call, queued for retry — the
    platform's equivalent of a DLQ for the partner->authority direction, with a
    replay process. Closes the "silently dropped outbound message" gap:
    previously a transient authority outage or an open circuit breaker meant
    ...
    # Correlation id threaded from the originating job/call so an abandoned
    # retry can still be traced back to the AgentJob that triggered it.
    correlation_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    # The envelope `message_id` every attempt of this row re-sends under — the
    # receiver's dedup key, for idempotent operations and safe
    # retries. Without it a retry looked like a new message and could be
    # processed twice by the authority when only the ACK, not the send, was lost.
...
class CertCaseExecution(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=_uuid)
    authority_change_id: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
