...
A bulkhead limit for a bounded resource: cost. Keeps LLM data access
cost-aware and resource use responsibly bounded.
...
class TokenBudgetExceeded(LlmBudgetExceededError):
    ...
    LLM token budget. Subclasses the taxonomy's LlmBudgetExceededError so a
...
def enforce_budget(db: Session, change_id: str, *, warn_at: float = 0.8) -> None:
    ...
    if spent >= budget:
        # Structured, alertable event. Budget exhaustion was previously visible
        # only as an HTTP 429 to whoever clicked the button — but it is a
        # cost-control AND abuse signal (a runaway loop, or a change being
        # driven far past its expected spend), so it needs to reach ops, not
        # just the UI.
