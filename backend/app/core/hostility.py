...
Every interface MUST be classified
...
class HostilityTier(str, Enum):
    ...
    H3 = "H3"  # external / partner-facing (authority A2A ingress/egress, LLM provider egress)
...
def _build_boundaries() -> dict[str, BoundaryLimits]:
    ...
    return {
        ...
        "authority_a2a_outbound": BoundaryLimits(
            name="authority_a2a_outbound",
            tier=HostilityTier.H3,
            max_request_bytes=10 * 1024 * 1024,
            timeout_connect_s=5.0,
            timeout_read_s=settings.authority_outbound_timeout_s,
            rate_limit_rps=0,  # outbound — governed by the authority's own ingress limits
            circuit_breaker_failure_threshold=settings.authority_cb_failure_threshold,
            circuit_breaker_cooldown_s=settings.authority_cb_cooldown_s,
            bulkhead_max_concurrent=settings.authority_outbound_max_concurrent,
            validation_strictness="strict",
        ),
...
def validate_at_startup() -> None:
    ...
    for name, b in boundaries.items():
        ...
        if name in ("authority_a2a_outbound", "llm_provider") and b.timeout_read_s <= 0:
