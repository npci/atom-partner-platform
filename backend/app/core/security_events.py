...
def emit_security_event(
    *,
    ...
    """Emit a structured security event. Logged at CRITICAL/WARNING/INFO so
    ...
    `actor` is WHO performed the action — a username for a dashboard-plane
    operation, omitted for the unauthenticated boundary events where there is
    no identity to record and where naming one would mean logging a
    credential-adjacent value.

    The field did not exist before, which meant no event in the system could
    carry an actor even where one was known and material: rotating
    `authority_hmac_secret` changes the key authenticating the entire A2A
    ingress, and the record of it could not say who did it. "Timestamp, actor
    and outcome" is the standard this catalogue is measured against and actor
    was the missing third.
    """
    payload = {
