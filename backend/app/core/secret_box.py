...
Crypto choice — Fernet, matching the authority backend
------------------------------------------------------
...
self-describing token. This is the EXACT mechanism the authority backend already
...
*self-chosen* `enc:v1:` prefix — the same tag the authority uses for a completely
...
thing across the platform, and follows the cryptographic-agility principle
of sharing frameworks or abstractions — a shared
...
protect authority signing secrets — trading a real security property for a
...
def _get_kek() -> bytes:
    ...
    if not raw:
        raise SecretBoxError(
            ...
            "This key encrypts authority_jwt_secret / authority_hmac_secret / partner_api_key / "
...
def validate_at_startup() -> None:
    """Refuse to boot on a KEK that is present but cannot work.

    `_get_kek` above states the contract — "a service that cannot encrypt its
    secrets must not start with a silent plaintext fallback" — but it is only
    ever reached from `_fernet()` at the first encrypt/decrypt. So the contract
    was documented and not implemented: a malformed KEK let this platform boot
    clean, serve its UI, pass health checks, and fail only when an operator
    saved their first credential, as an opaque 500 whose real cause appeared
    solely in the container log. That is the worst possible moment to discover
    it, because the operator is mid-install and has no reason to suspect a
    value they set weeks earlier. See F-25.

    The two failure modes are NOT the same and are treated differently:

      * PRESENT BUT UNUSABLE (wrong decoded length, or not base64) is FATAL.
        It cannot be deliberate, no secret can be written or read while it
        holds, and storing authority credentials is this platform's entire
        job. Refusing to start is strictly kinder than appearing healthy.

      * UNSET only warns. `docker-compose.yml` defaults PARTNER_SECRET_KEK to
        empty, so making this fatal would break a stock `docker compose up`
        for anyone evaluating the platform who never stores a secret. They
        still get a CRITICAL line naming the variable at boot, and `_get_kek`'s
        existing error the moment they try to save.

    Called from `main.py`'s startup hook alongside the other validators, and
    BEFORE init_db() — same rule as `core/hostility.validate_at_startup`.
    """
    raw = (os.environ.get(_KEK_ENV_VAR) or _kek_from_settings() or "").strip()
    if not raw:
        logger.critical(
            "%s is not set. The platform will start, but EVERY attempt to save "
            "or read a credential in Settings will fail. Generate one with: "
    ...
    try:
        ...
        # Re-raised as RuntimeError, not SecretBoxError, to match the shape the
        # other startup validators use and so an operator sees a boot failure
        # rather than a library-specific exception.
        raise RuntimeError(
            f"{_KEK_ENV_VAR} is set but unusable, so no credential could ever be "
            f"stored or read: {exc}"
        ) from exc

    logger.info("%s validated: decodes to 32 bytes.", _KEK_ENV_VAR)
...
def encrypt(plaintext: str) -> str:
    ...
    secret would be silently unrecoverable. The authority implementation carries
...
def decrypt(stored: str) -> str:
    ...
    value to carry on using. (This differs deliberately from the authority
    implementation, which logs and returns "" — here a corrupted authority signing
...
# auth_middleware, authority_client, code_ingestion), and the migration script —
...
# a `<something>_secret = "<literal>"` position that a hardcoded-credential
# scanner reads as an embedded credential. Membership,
...
def safe_key_label(key: str) -> str:
    ...
    --------------------------------------
    The key name ("authority_hmac_secret") is a harmless identifier, but to a
    ...
    authority_hmac_secret". A sensitive-log-filtering scan rule therefore
    ...
    `_SECRET_KEY_LABELS` as the tainted object. The reason is that such engines
    model a dictionary read as a taint PROPAGATOR: a tainted key indexing a
    ...
    and the flow `"authority_hmac_secret"` → `_get_setting(key)` → `safe_key_label`
    ...
    `"authority_jwt_secret"`-style literal for the "Use Of Hardcoded Password" query
    to latch onto either. `SettingKey.authority_jwt_secret == "authority_jwt_secret"` is
    ...
    """
    if key == SettingKey.authority_jwt_secret:
        return "the authority JWT secret"
    if key == SettingKey.authority_hmac_secret:
        return "the authority HMAC secret"
    if key == SettingKey.partner_api_key:
        return "the authority-issued partner API key"
    if key == SettingKey.partner_anthropic_api_key:
        return "the LLM provider API key"
    if key == SettingKey.partner_openai_api_key:
        return "the LLM provider API key"
    if key == SettingKey.partner_ainxt_api_key:
        return "the LLM provider API key"
    if key == SettingKey.gitlab_token:
        return "the GitLab access token"
    if key == SettingKey.cert_trigger_secret:
        return "the certification trigger secret"
