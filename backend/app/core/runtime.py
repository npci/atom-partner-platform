...
Closes two architectural-principle gaps that the earlier review did not cover
...
platform, not one to impose here.
"""
...
def drain(timeout_s: float, poll_interval_s: float = 0.1) -> tuple[int, float]:
    ...
    path. This is not a `thread.sleep` coordination anti-pattern — it is a
    bounded terminal wait with a deadline, not inter-thread signalling.
    """
    stop_accepting()
