...
This is **reference base code** for organisations participating in a governed ecosystem to
interact with an authority's Change Management Platform over the A2A protocol — and to **plug
in their own agents**. Fork it, configure it, replace the agent bodies with your logic.

The domain is configuration, not code: `DOMAIN_PACK` selects a pack under
`backend/app/packs/` supplying roles, identifiers and prompt vocabulary. Packs ship for
UPI/NPCI (`upi`), inter-library lending (`nlln`) and a neutral default (`generic`).

> The platform is a **client** of a live authority instance. It needs a reachable authority
> A2A endpoint plus authority-issued credentials (configured from the Settings UI). It is
...
```
TIER 1  CONTRACT   app/a2a_common/*   ── the authority A2A wire (JSON-RPC, JWT, HMAC).
                                          DO NOT EDIT. Keep in sync with the authority
                                          on releases. Header names and method ids on
                                          this wire stay `X-NPCI-*` / `npci.*` — they are
                                          matched byte-for-byte by the counterparty.
...
hosted via `url:` so those clients and secrets live in your process, not the shipped one.
...
| Runtime operator settings (authority URLs, authority-issued API key / JWT / HMAC secrets) | `partner_settings` DB table | Edited from the Settings UI |
...
Every `execute()` writes a row to `agent_runs` (partner-side mirror of the authority's `agent_jobs`):
...
`code/review` → `code/fix` → `code/merge-request`)
...
Standalone (no authority monorepo needed):
...
# Windows + Git Bash: prefix with MSYS_NO_PATHCONV=1, or the shell rewrites
# -subj into a Windows path and OpenSSL rejects it. See DEPLOYMENT_GUIDE.md §3.1.
mkdir -p deploy/tls
...
On first login, open **Settings** and enter your authority Platform URL and
authority-issued credentials. With no LLM key, the feasibility agent returns mock
...
(the standalone compose ships `data/partner_profile.template.md`; worked examples for two
different domains are in `data/examples/`), or set `PARTNER_PROFILE_PATH`.
...
- The authority↔partner A2A wire (Tier 1) is unchanged.
